...
function ChangePassword() {
  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      ...
      case "passwordMatch":
        ...
        return
      case "submitForm":
        if (!draft.username.hasErrors && !draft.username.isUnique && !draft.password.hasErrors && !draft.cfmpassword.hasErrors) {
          draft.submitCount++
        }
        return
    }
  ...
  useEffect(() => {
    if (state.submitCount) {
      async function fetchResults() {
        try {
          const response = await Axios.post("/changePassword", { username: state.username.value, password: state.password.value, cfmpassword: state.cfmpassword.value }, { withCredentials: true })
